...
if __name__ == "__main__":
    ...
    for metric_idx, metric_name in enumerate(data_comparison_Trajectory["metrics"]):
        ...
        for dataset_idx, dataset_name in enumerate(
            data_comparison_Trajectory["datasets"]
        ):
            ax.bar(
            ...
            xtick_list.append(
                np.mean(np.arange(num_methods)) + dataset_idx * (num_methods + 1)
            )
        ...
        ax.set_ylabel(
            data_comparison_Trajectory["metrics"][metric_idx], fontsize=36, labelpad=12
        )
