...
data_math_correcting_llm = {
    ...
    "result": {
        ...
        r"Degenerate repetition or stuck$\downarrow$": np.array([9, 2, 4, 0, 0, 0])
        / 14,
...
data_math_correcting_human = {
    ...
    "result": {
        ...
        r"Degenerate repetition or stuck$\downarrow$": np.array([5, 3, 2, 0, 0, 0])
        / 14,
...
if __name__ == "__main__":
    ...
    for subtype_idx, subtype_name in enumerate(data_math_correcting_llm["subtypes"]):
        ...
        ax.set_title(
            data_math_correcting_llm["subtypes"][subtype_idx], fontsize=30, pad=36
        )
    ...
    for subtype_idx, subtype_name in enumerate(data_math_correcting_human["subtypes"]):
        ...
        ax.set_title(
            data_math_correcting_human["subtypes"][subtype_idx], fontsize=30, pad=36
        )
        ax.set_ylabel("Probability", fontsize=30, labelpad=12)
        ax.set_ylim([0, 1])
        ax.set_xticks([])
