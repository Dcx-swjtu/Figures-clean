...
if __name__ == "__main__":
    ...
    if PLOT_DE_NOVO_SPEED:
        ax.bar(
            np.arange(len(options)),
            1 / np.array(results_inference),
            color=colors,
            label=options,
        )
        ax.hlines(
            y=1 / results_inference[-1],
            xmin=-1,
            xmax=len(options),
            color=colors[-1],
            linestyle="--",
        ...
        ax.set_xticks([])
        ax.set_ylabel(
            "Inference Throughput " + r"$\uparrow$" + "\n(samples / ms)", fontsize=18
        )
        ...
        ax.bar(
            np.arange(len(options_speed)),
            speed_values,
            color=colors[4:8] + [colors[-1]],
            label=options_speed,
        )
    ...
    for j in range(n_cols):
        ...
        norm = mpl.colors.Normalize(
            vmin=vmin[j] if j % 2 == 1 else 0, vmax=vmax[j] if j % 2 == 0 else 0
        )
        ax.imshow(
    ...
    for (i, j), val in np.ndenumerate(improvements):
        if i == n_rows - 1:
            ...
            fontsize = 14
        ax.text(
            j, i + 0.5, label, ha="center", va="center", fontsize=fontsize, color=color
        )
    ...
    pct_improvement_over_best = (
        100 * (percentages[-1] - best_percentages) / best_percentages
    )
