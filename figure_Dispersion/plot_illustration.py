...
def nice_axes(ax, L):
    ...
    ax.quiver(
        0, 0, 0, y_scale * L, 0, 0, color="black", linewidth=2, arrow_length_ratio=0.1
    )
    ax.quiver(
        0, 0, 0, 0, 0, z_scale * L, color="black", linewidth=2, arrow_length_ratio=0.1
    )
...
def _slerp_arc(p, q, n=200):
    ...
    s = np.sin
    arc = (s((1 - t) * theta)[:, None] * p + s(t * theta)[:, None] * q) / (
        s(theta) + EPSILON
    )
...
def plot_decorrelation(ax):
    ...
    intensity = np.maximum(
        0.0, nx * light_dir[0] + ny * light_dir[1] + nz * light_dir[2]
    )
    ...
    light_dir = np.array([0.5, 0.5, -0.8])
    light_dir /= np.linalg.norm(light_dir)
    intensity_e = np.maximum(
        0.0, nxE * light_dir[0] + nyE * light_dir[1] + nzE * light_dir[2]
    )
...
def plot_orthogonalization(ax):
    ...
    ax.text(
        ...
        va="center",
        bbox=dict(
            facecolor="white", alpha=1, edgecolor="none", boxstyle="round,pad=0.2"
        ),
    )
    ax.text(
        -0.6,
